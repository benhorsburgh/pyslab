...
from pyslab.puzzle_grid import is_solved, has_unique_solution
...
class TestPermuteRowBlocks:
    ...
    def test_all_row_blocks_present(self, simple_puzzle):
        ...
        assert all(
            (simple_puzzle[r,:] == permuted_puzzle).all(axis=1).any()
            for r in range(3)
        )
...
class TestPermuteColBlocks:
    ...
    def test_all_col_blocks_present(self, simple_puzzle):
        ...
        assert all(
            (simple_puzzle[c,:] == permuted_puzzle).all(axis=1).any()
            for c in range(3)
        )
